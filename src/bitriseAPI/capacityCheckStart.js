...
//Get builds that are currently running, in how much time will the build end, and return the number of sessions will use when my builds will start.
    ...
    const filteredBuilds = buildsRunning.filter((build) => Object.keys(RESERVED_SESSIONS).includes(build.triggered_workflow));
...
//Get the average time of the builds the user wants to run and how many sessions should they use.
    ...
    if (occupiedSessions + sessionsYouWillNeed > SESSIONS_LIMIT) {
        console.log(CONSOLE_RED, `Workflow ${WORKFLOW} will need ${sessionsYouWillNeed} sessions, so it should not be triggered at the moment :( Estimated sessions (with your build) ${occupiedSessions + sessionsYouWillNeed}`);
    } else {
        console.log(CONSOLE_GREEN, `${WORKFLOW} will need ${sessionsYouWillNeed} sessions, so it can be triggered!!!`);
