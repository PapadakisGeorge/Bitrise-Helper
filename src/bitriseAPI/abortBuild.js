...
const findBuildNumber = async () => {
    ...
    while (!buildNumber) {
        let buildNumberKnownInput = readline.question('Do you know the build number of the build you want to abort? (y/n):\n', {
        ...
        if (YES_OPTIONS.includes(buildNumberKnownInput)) {
            while(!buildNumber){
                buildNumber=readline.question('Please enter the build you want to abort:\n');
            }
            return builds.filter((build) => build.buildNumber == buildNumber)[0].buildSlug;
        } else if (NO_OPTIONS.includes(buildNumberKnownInput)) {
            const buildNumbers = builds.map((build) => build.buildNumber);
            let buildNumberInput = readline.keyInSelect(buildNumbers, `Please select the build you want to abort`);
            if(buildNumberInput === -1) process.exit(0);
            return builds.filter((build) => build.buildNumber == buildNumbers[buildNumberInput])[0].buildSlug;
        }
    }
}
